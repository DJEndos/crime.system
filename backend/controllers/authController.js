...
async function register(req, res) {
  const { full_name, username, password, role, badge_number, rank, phone, station } = req.body;
  ...
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO users (full_name, username, password_hash, role, badge_number, rank, phone, station)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [full_name, username, hash, role, badge_number, rank || null, phone || null, station || 'Ikot Udota Division, Eket']
    );
...
async function listOfficers(req, res) {
  try {
    const [rows] = await pool.query(
      `SELECT id, full_name, badge_number, role, rank, station FROM users WHERE is_active = 1 ORDER BY full_name`
    );
